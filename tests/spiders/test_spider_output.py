...
def test_spider_produces_valid_items(spider_name, tmp_path):
    ...
    assert result.returncode == 0, (
        f"{spider_name}: el crawl falló (exit code {result.returncode})\n"
        f"stderr:\n{result.stderr}"
    )

    assert output_file.exists(), f"{spider_name}: no se generó ningún archivo de salida"

    lines = output_file.read_text(encoding='utf-8').splitlines()
    assert len(lines) >= 1, f"{spider_name}: no se scrapeó ningún item"

    item = json.loads(lines[0])["raw_payload"]

    missing = [k for k in required_keys if not has_nested_key(item, k)]
    assert not missing, f"{spider_name}: keys faltantes o en null: {missing}"