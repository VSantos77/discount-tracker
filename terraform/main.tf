...
# ---------------------------------------------------------------------------
# Log-based metrics — scrapy WORKFLOW_STATS per spider run
# ---------------------------------------------------------------------------
...
locals {
  scrapy_log_filter = join("\n", [
    ...
    "logName=\"projects/vocal-tracer-484119-t7/logs/run.googleapis.com%2Fstderr\"",
...
resource "google_logging_metric" "scrapy_items_scraped" {
  ...
  label_extractors = {
    spider = "REGEXP_EXTRACT(textPayload, \"spider[^a-z]+([a-z]+)\")"
  }
  bucket_options {
    exponential_buckets {
